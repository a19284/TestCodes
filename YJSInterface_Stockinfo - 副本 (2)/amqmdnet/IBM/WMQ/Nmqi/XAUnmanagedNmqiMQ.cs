namespace IBM.WMQ.Nmqi
{
    ...
    internal class XAUnmanagedNmqiMQ : MQBase, NmqiMQ, NmqiSP
    {
        private object asyncCallbackLock = new object();
        private Hashtable asyncHobjsXA = new Hashtable();
        ...
        static XAUnmanagedNmqiMQ()
        {
            lock (initialiseLock)
            {
                if (nativeManager == null)
                {
                    ...
                    nativeManager.InitializeNativeApis("MQSeries XA Client");
                }
            }
        }

        public XAUnmanagedNmqiMQ(NmqiEnvironment env, int options)
        {
            base.TrConstructor("%Z% %W%  %I% %E% %U%", new object[] { env, options });
            this.env = env;
            this.mqiOptions = options;
            this.myDelegate = new MQBase.CallbackDelegate(this.NmqiConsumerMethodXA);
        }
        ...
        public void MQCB(Hconn hConn, int operation, MQCBD pCallbackDesc, Hobj hobj, MQMessageDescriptor pMsgDesc, MQGetMessageOptions getMsgOpts, out int pCompCode, out int pReason)
        {
            ...
            try
            {
                ...
                if ((operation & 0x100) != 0)
                {
                    ...
                    {
                        if ((pCallbackDesc.CallbackType == 1) && !this.asyncHobjsXA.Contains(key))
                        {
                            this.asyncHobjsXA.Add(key, localHobj);
            ...
            Label_0175:
                ...
                nativeManager.NativeACMQCB(num3, operation, key, ptr, zero, ptr3, this.myDelegate, 1, out pCompCode, out pReason);
                if (pCompCode == 0)
                {
        ...
        public void MQCONN(string pQMgrName, Phconn phconn, out int pCompCode, out int pReason)
        {
            ...
            {
                base.TrText("Ignoring Win32Exception");
            ...
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        this.SPIQuerySPI(unmanagedHconn, out pCompCode, out pReason);
        ...
        public void NmqiConnect(string name, NmqiConnectOptions pNmqiConnectOpts, MQConnectOptions cno, Hconn parentHconn, Phconn pHconn, out int pCompCode, out int pReason)
        {
            ...
            try
            {
                ...
                if (pReason == 0)
                {
                    ...
                    if ((pCompCode == 0) || (pCompCode == 1))
                    {
                        unmanagedHconn.Value = num2;
                        unmanagedHconn.UpdateHconn(this, pHconn);
                        unmanagedHconn.Parent = parentHconn;
                        unmanagedHconn.PHconn = pHconn;
                        cno.cd.StructMQCD = (MQBase.MQCD) Marshal.PtrToStructure(structMQCNO.ClientConnPtr, typeof(MQBase.MQCD));
                        this.SPIQuerySPI(unmanagedHconn, out pCompCode, out pReason);
                        if ((pCompCode == 0) || (pCompCode == 1))
                        {
        ...
        public void NmqiConsumerMethodXA(int hconn, IntPtr structMqmd, IntPtr structMqgmo, IntPtr buffer, IntPtr structMqcbc)
        {
            uint method = 0x443;
            ...
            try
            {
                ...
                if (mqcbc != null)
                {
                    lock (this.asyncCallbackLock)
                    {
                        if (this.asyncHobjsXA.Contains(mqcbc.HOBJ))
                        {
                            hobj = (UnmanagedHobj) this.asyncHobjsXA[mqcbc.HOBJ];
        ...
        internal void SPIQuerySPI(Hconn hConn, out int pCompCode, out int pReason)
        {
            uint method = 0x3fd;
            this.TrEntry(method, new object[] { hConn, "pCompCode : out", "pReason : out" });
            pCompCode = 0;
        ...
        public void zstMQGET(Hconn hConn, Hobj hObj, ref MQMessageDescriptor md, ref MQGetMessageOptions gmo, int bufferLength, byte[] buffer, out int dataLength, MQLPIGetOpts lpiGetOpts, out int pCompCode, out int pReason)
        {
            ...
            try
            {
                ...
                int num3 = localHobj.Value;
                int pSConvs = 0;
                this.GetSharingConversationsValue(num2, ref pSConvs);
                if (pSConvs > 0)
                {
                    ...
                    nativeManager.zstMQGET(num2, num3, ref structMQMD, ref structMQGMO, bufferLength, buffer, out dataLength, ref structGETOPT, out pCompCode, out pReason);
                ...
                else
                {
                    ...
                    nativeManager.MQGET(num2, num3, ref mqmd, ref mqgmo, bufferLength, buffer, out dataLength, out pCompCode, out pReason);
                    md.StructMQMD = mqmd;
