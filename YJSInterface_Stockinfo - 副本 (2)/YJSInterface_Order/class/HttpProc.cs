...
namespace YJSInterface
{
    ...
    public class WebClient
    {
        ...
        {
            //Console.WriteLine("0.1请求地址:"+url);
            byte[] data = encoding.GetBytes(postData);
            //Console.WriteLine("0.2请求地址:" + url);
        ...
        public string Post(string url, byte[] postData)
        {
            ...
            //    Console.WriteLine("0.3请求地址:" + url);
            long startTime = DateTime.Now.Ticks;
            HttpWebRequest request = CreateRequest(url, "POST");
            //Console.WriteLine("0.4请求地址:" + url);
            request.Proxy = null;
            long baidurespHtmlEndTime = DateTime.Now.Ticks;
            double time = (baidurespHtmlEndTime - startTime) / 10000000.0;
            Console.WriteLine("1:CreateRequest时间:{0}", time);
            ...
            PostData(request, postData);
             ...
             baidurespHtmlEndTime = DateTime.Now.Ticks;
             time = (baidurespHtmlEndTime - startTime) / 10000000.0;
             Console.WriteLine("2:请求时间:{0}" , time);

          startTime = DateTime.Now.Ticks;

           byte[] dataRequest= GetData(request);
           baidurespHtmlEndTime = DateTime.Now.Ticks;
           time = (baidurespHtmlEndTime - startTime) / 10000000.0;
           Console.WriteLine("3:读取时间:{0}" , time);

           startTime = DateTime.Now.Ticks;
           respHtml = encoding.GetString(dataRequest);
            baidurespHtmlEndTime = DateTime.Now.Ticks;
            time = (baidurespHtmlEndTime - startTime) / 10000000.0;
            Console.WriteLine("4:读取encoding.GetString时间:{0}", time);
        ...
        private byte[] GetData(HttpWebRequest request)
        {
            ...
            Console.WriteLine("GetResponse时间:{0}", time);
            ...
            while ((count = stream.Read(buf, 0, buf.Length)) > 0)
            {
             ...
             baidurespHtmlEndTime = DateTime.Now.Ticks;
             time = (baidurespHtmlEndTime - startTime) / 10000000.0;
            Console.WriteLine("读取到MemoryStream时间:{0}", time);
            response.Close();
            System.GC.Collect();
            //解压    
           ...
           baidurespHtmlEndTime = DateTime.Now.Ticks;
           time = (baidurespHtmlEndTime - startTime) / 10000000.0;
           Console.WriteLine("ToArray时间:{0}", time);
           System.GC.Collect();
           return temp;
        }
        /// <summary>    
